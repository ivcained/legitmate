...
type LaunchState = { template: string; phase: 'launching' | 'applied' | 'failed'; message: string; requestId: string; instance?: AgentInstance; receipt?: ConfigurationReceipt; agencyAgent?: AgencyAgent }
  ...
  const launchAgent = async (template: string, label: string, resources = resourceConfig) => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        ...
        setLaunchState({ template, phase: 'failed', requestId, message: result.message ?? (failedInstance ? 'The workspace exists, but its configuration could not be verified.' : 'Workspace creation failed.'), instance: failedInstance })
        return
      }
